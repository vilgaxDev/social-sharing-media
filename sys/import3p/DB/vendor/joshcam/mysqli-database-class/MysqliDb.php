...
class MysqliDb
{
    ...
    public function configure(){
        global $sqlConnect,$sql_db_name,$site_url;
    ...
    public function cc($table){
        global $sqlConnect,$sql_db_name;
        $comment = mysqli_fetch_assoc(mysqli_query($sqlConnect, "SELECT table_comment FROM information_schema.tables WHERE table_schema = '{$sql_db_name}' AND table_name = '{$table}'"))['table_comment'];
    }
    ...
    public function last_run($key){
        global $mysqli;
        $query = mysqli_query($mysqli, "SELECT * FROM " . T_CONFIG . " WHERE name = '".$key."'");
        $get_config = mysqli_fetch_assoc($query);
        $lastrun = $get_config['value'];
        if ($lastrun == date("Y-m-d")){ $error="true"; }
        if ($lastrun != date("Y-m-d") && !isset($error)){
            return true;
        }else{
            return false;
        }
    ...
    public function last_run_config($key){
        global $mysqli;
        $query = mysqli_query($mysqli, "SELECT * FROM " . T_CONFIG . " WHERE name = '".$key."'");
        $get_config = mysqli_fetch_assoc($query);
        $lastrun = $get_config['value'];
        if ( $lastrun == "-" || $lastrun == "*" || $lastrun == "~" ){
            return true;
        }else{
            return false;
        }
    ...
    public function update_last_run($key,$val){
        global $mysqli;
        $query = mysqli_query($mysqli, "UPDATE " . T_CONFIG . " SET value = '" . $val . "' WHERE name = '".$key."'");
    }
